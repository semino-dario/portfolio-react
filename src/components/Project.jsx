    ...
    <div className="container flex flex-col  w-72 shrink-0 mx-auto">
      {imagenes === null ? (
        ...
        <div className="flex flex-col items-center rounded border-2 border-titulo bg-verde mb-[15px] ">
          {imagenes ? (
            <SwipeableTextMobileStepper imagenes={imagenes} />
          ) : (
      ...
      <div
        ...
        className={`rounded-xl h-[50px] w-full bg-azulMedio flex items-center mt-[15px] justify-around cursor-pointer ${
          !techInfo && "hidden"
        }`}
      >
        <p style={{ color: "black" }}> Technical information</p>{" "}
        <p>{!openPanel2 ? "+" : "-"}</p>
      </div>
      <Collapse
        className="ReactCollapse--collapse"
        isOpened={!openPanel2 ? false : true}
      >
